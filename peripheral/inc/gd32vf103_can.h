#ifndef __GD32VF103_CAN_H
#define __GD32VF103_CAN_H
...
// CAN filter associated FIFO register
#define CAN_FAFIFO(canx)			REG32((canx) + 0x214U)

// CAN filter working register
#define CAN_FW(canx)				REG32((canx) + 0x21CU)

// CAN filter 0 data 0 register
#define CAN_F0DATA0(canx)			REG32((canx) + 0x240U)
// CAN filter 1 data 0 register
...
#define CAN_TMI(canx, bank) \
	REG32((canx) + 0x180U + ((bank) * 0x10U))

// CAN transmit mailbox property register
#define CAN_TMP(canx, bank) \
	REG32((canx) + 0x184U + ((bank) * 0x10U))

// CAN transmit mailbox data0 register
#define CAN_TMDATA0(canx, bank) \
	REG32((canx) + 0x188U + ((bank) * 0x10U))
// CAN transmit mailbox data1 register
#define CAN_TMDATA1(canx, bank) \
	REG32((canx) + 0x18CU + ((bank) * 0x10U))


// CAN filter bank
// CAN filter data 0 register
#define CAN_FDATA0(canx, bank) \
	REG32((canx) + 0x240U + ((bank) * 0x8U) + 0x0U)
// CAN filter data 1 register
#define CAN_FDATA1(canx, bank) \
	REG32((canx) + 0x240U + ((bank) * 0x8U) + 0x4U)


// CAN receive fifo mailbox bank
// CAN receive FIFO mailbox identifier register
#define CAN_RFIFOMI(canx, bank) \
	REG32((canx) + 0x1B0U + ((bank) * 0x10U))

// CAN receive FIFO mailbox property register
#define CAN_RFIFOMP(canx, bank) \
	REG32((canx) + 0x1B4U + ((bank) * 0x10U))

// CAN receive FIFO mailbox data0 register
#define CAN_RFIFOMDATA0(canx, bank) \
	REG32((canx) + 0x1B8U + ((bank) * 0x10U))

// CAN receive FIFO mailbox data1 register
#define CAN_RFIFOMDATA1(canx, bank) \
	REG32((canx) + 0x1BCU + ((bank) * 0x10U))
...
// CAN_TSTAT
// mailbox0 transmit finished
#define CAN_TSTAT_MTF0				BIT(0)

// mailbox0 transmit finished and no error
#define CAN_TSTAT_MTFNERR0			BIT(1)

// mailbox0 arbitration lost
#define CAN_TSTAT_MAL0				BIT(2)

// mailbox0 transmit error
#define CAN_TSTAT_MTE0				BIT(3)

// mailbox0 stop transmitting
#define CAN_TSTAT_MST0				BIT(7)

// mailbox1 transmit finished
#define CAN_TSTAT_MTF1				BIT(8)

// mailbox1 transmit finished and no error
#define CAN_TSTAT_MTFNERR1			BIT(9)

// mailbox1 arbitration lost
#define CAN_TSTAT_MAL1				BIT(10)

// mailbox1 transmit error
#define CAN_TSTAT_MTE1				BIT(11)

// mailbox1 stop transmitting
#define CAN_TSTAT_MST1				BIT(15)

// mailbox2 transmit finished
#define CAN_TSTAT_MTF2				BIT(16)

// mailbox2 transmit finished and no error
#define CAN_TSTAT_MTFNERR2			BIT(17)

// mailbox2 arbitration lost
#define CAN_TSTAT_MAL2				BIT(18)

// mailbox2 transmit error
#define CAN_TSTAT_MTE2				BIT(19)

// mailbox2 stop transmitting
#define CAN_TSTAT_MST2				BIT(23)

// mailbox number
#define CAN_TSTAT_NUM				BITS(24,25)

// transmit mailbox0 empty
#define CAN_TSTAT_TME0				BIT(26)

// transmit mailbox1 empty
#define CAN_TSTAT_TME1				BIT(27)

// transmit mailbox2 empty
#define CAN_TSTAT_TME2				BIT(28)

// last sending priority flag for mailbox0
#define CAN_TSTAT_TMLS0				BIT(29)

// last sending priority flag for mailbox1
#define CAN_TSTAT_TMLS1				BIT(30)

// last sending priority flag for mailbox2
...
// CAN_FSCFG
// filter scale, 32 bits or 16 bits
...
#define CAN_REGIDX_BIT(regidx, bitpos) \
	(((uint32_t) (regidx) << 6) | (uint32_t) (bitpos))

#define CAN_REG_VAL(canx, offset) \
	(REG32((canx) + ((uint32_t) (offset) >> 6)))

#define CAN_BIT_POS(val) \
	((uint32_t) (val) & 0x1FU)


#define CAN_REGIDX_BITS(regidx, bitpos0, bitpos1) \
	(((uint32_t) (regidx) << 12) | ((uint32_t) (bitpos0) << 6) | \
	 (uint32_t) (bitpos1))

#define CAN_REG_VALS(canx, offset) \
	(REG32((canx) + ((uint32_t) (offset) >> 12)))

#define CAN_BIT_POS0(val) \
	(((uint32_t) (val) >> 6) & 0x1FU)

#define CAN_BIT_POS1(val) \
	((uint32_t) (val) & 0x1FU)
...
enum can_flag {
	...
	// status change flag of entering sleep working mode
	CAN_FLAG_SLPIF = CAN_REGIDX_BIT(STAT_REG_OFFSET, 4U),
	// status change flag of wakeup from sleep working mode
...
enum can_interrupt_flag {
	...
	// mailbox 2 transmit finished interrupt flag
	CAN_INT_FLAG_MTF2 = CAN_REGIDX_BITS(TSTAT_REG_OFFSET, 16U, 0U),
	// mailbox 1 transmit finished interrupt flag
...
struct can_param {
	...
	enum control_status time_triggered;
	// automatic bus-off recovery
	enum control_status auto_bus_off_recovery;
	// automatic wake-up mode
	enum control_status auto_wake_up;
	// automatic retransmission mode disable
	enum control_status no_auto_retrans;
	// receive FIFO overwrite mode
	enum control_status rec_fifo_overwrite;
	// transmit FIFO order
	enum control_status trans_fifo_order;
	// baudrate prescaler
	uint16_t prescaler;
};
...
struct can_filter_param {
	// filter list number high bits
	uint16_t filter_list_high;
	// filter list number low bits
	uint16_t filter_list_low;
	// filter mask number high bits
	uint16_t filter_mask_high;
	// filter mask number low bits
	uint16_t filter_mask_low;
	// receive FIFO associated with the filter
	...
	// filter work or not
	enum control_status filter_enable;
};
...
// transmit states
enum can_transmit_state {
	// CAN transmitted failure
	CAN_TRANSMIT_FAILED = 0U,
	// CAN transmitted success
	CAN_TRANSMIT_OK = 1U,
	// CAN transmitted pending
	CAN_TRANSMIT_PENDING = 2U,
	// no empty mailbox to be used for CAN
	CAN_TRANSMIT_NOMAILBOX = 4U,
};

enum can_struct_type {
	// CAN initiliaze parameters struct
	CAN_INIT_STRUCT = 0,
	// CAN filter parameters struct
	CAN_FILTER_STRUCT,
...
// CAN baudrate prescale
#define BT_BAUDPSC(regval) \
	(BITS(0,9) & ((uint32_t) (regval) << 0))


// CAN bit segment 
#define BT_BS1(regval) \
	(BITS(16,19) & ((uint32_t) (regval) << 16))

#define BT_BS2(regval) \
	(BITS(20,22) & ((uint32_t) (regval) << 20))


// CAN resynchronization jump widt
#define BT_SJW(regval) \
	(BITS(24,25) & ((uint32_t) (regval) << 24))


// CAN communication mod
#define BT_MODE(regval) \
	(BITS(30,31) & ((uint32_t) (regval) << 30))


// CAN FDATA high 16 bits
#define FDATA_MASK_HIGH(regval) \
	(BITS(16,31) & ((uint32_t) (regval) << 16))
// CAN FDATA low 16 bits
#define FDATA_MASK_LOW(regval) \
	(BITS(0,15) & ((uint32_t) (regval) << 0))

// CAN1 filter start bank_numbe
#define FCTL_HBC1F(regval) \
	(BITS(8,13) & ((uint32_t) (regval) << 8))


// CAN transmit mailbox extended identifie
#define TMI_EFID(regval) \
	(BITS(3,31) & ((uint32_t) (regval) << 3))


// CAN transmit mailbox standard identifier
#define TMI_SFID(regval) \
	(BITS(21,31) & ((uint32_t) (regval) << 21))


// transmit data byte 0
#define TMDATA0_DB0(regval) \
	(BITS(0,7) & ((uint32_t) (regval) << 0))
// transmit data byte 1
#define TMDATA0_DB1(regval) \
	(BITS(8,15) & ((uint32_t) (regval) << 8))
// transmit data byte 2
#define TMDATA0_DB2(regval) \
	(BITS(16,23) & ((uint32_t) (regval) << 16))
// transmit data byte 3
#define TMDATA0_DB3(regval) \
	(BITS(24,31) & ((uint32_t) (regval) << 24))
// transmit data byte 4
#define TMDATA1_DB4(regval) \
	(BITS(0,7) & ((uint32_t) (regval) << 0))
// transmit data byte 5
#define TMDATA1_DB5(regval) \
	(BITS(8,15) & ((uint32_t) (regval) << 8))
// transmit data byte 6
#define TMDATA1_DB6(regval) \
	(BITS(16,23) & ((uint32_t) (regval) << 16))
// transmit data byte 7
#define TMDATA1_DB7(regval) \
	(BITS(24,31) & ((uint32_t) (regval) << 24))

// receive mailbox extended identifie
#define GET_RFIFOMI_EFID(regval) \
	GET_BITS((uint32_t) (regval), 3U, 31U)

// receive mailbox standrad identifie
#define GET_RFIFOMI_SFID(regval) \
	GET_BITS((uint32_t) (regval), 21U, 31U)

// receive data length
#define GET_RFIFOMP_DLENC(regval) \
	GET_BITS((uint32_t) (regval), 0U, 3U)

// the index of the filter by which the frame is passed
#define GET_RFIFOMP_FI(regval) \
	GET_BITS((uint32_t) (regval), 8U, 15U)

// receive data byte 0
#define GET_RFIFOMDATA0_DB0(regval) \
	GET_BITS((uint32_t) (regval), 0U, 7U)
// receive data byte 1
#define GET_RFIFOMDATA0_DB1(regval) \
	GET_BITS((uint32_t) (regval), 8U, 15U)
// receive data byte 2
#define GET_RFIFOMDATA0_DB2(regval) \
	GET_BITS((uint32_t) (regval), 16U, 23U)
// receive data byte 3
#define GET_RFIFOMDATA0_DB3(regval) \
	GET_BITS((uint32_t) (regval), 24U, 31U)
// receive data byte 4
#define GET_RFIFOMDATA1_DB4(regval) \
	GET_BITS((uint32_t) (regval), 0U, 7U)
// receive data byte 5
#define GET_RFIFOMDATA1_DB5(regval) \
	GET_BITS((uint32_t) (regval), 8U, 15U)
// receive data byte 6
#define GET_RFIFOMDATA1_DB6(regval) \
	GET_BITS((uint32_t) (regval), 16U, 23U)
// receive data byte 7
#define GET_RFIFOMDATA1_DB7(regval) \
	GET_BITS((uint32_t) (regval), 24U, 31U)


// error number
#define GET_ERR_ERRN(regval) \
	GET_BITS((uint32_t) (regval), 4U, 6U)

// transmit error count
#define GET_ERR_TECNT(regval) \
	GET_BITS((uint32_t) (regval), 16U, 23U)

// receive  error count
#define GET_ERR_RECNT(regval) \
	GET_BITS((uint32_t) (regval), 24U, 31U)

// CAN errors
#define ERR_ERRN(regval) \
	(BITS(4,6) & ((uint32_t) (regval) << 4))
...
// mask for frame number in receive FIFOx
#define CAN_RFIF_RFL_MASK			((uint32_t)0x00000003U)

// mask of standard identifier
#define CAN_SFID_MASK				((uint32_t)0x000007FFU)
// mask of extended identifier
