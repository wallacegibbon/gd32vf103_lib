...
void timer_quadrature_decoder_mode_config(uint32_t timer_periph,
	...
	TIMER_CHCTL0(timer_periph) &= ~TIMER_CHCTL0_CH0MS & ~TIMER_CHCTL0_CH1MS;
	TIMER_CHCTL0(timer_periph) |=
		TIMER_IC_SELECTION_DIRECTTI | TIMER_IC_SELECTION_DIRECTTI << 8;
...
void timer_external_trigger_as_external_clock_config(uint32_t timer_periph,
	...
	if (extrigger == TIMER_SMCFG_TRGSEL_CI1FE1) {
		...
		// reset the CH1NP bit
		TIMER_CHCTL2(timer_periph) &=
			~(TIMER_CHCTL2_CH1P | TIMER_CHCTL2_CH1N);
		// set the CH1NP bit
...
void timer_external_clock_mode0_config(uint32_t timer_periph,
	...
	TIMER_SMCFG(timer_periph) &=
		~(TIMER_SMCFG_SMC | TIMER_SMCFG_TRGS);
 ...
 * 	only one parameter can be selected which is shown as below:
 * 		TIMER_INT_FLAG_UP:
 * 			update interrupt flag, TIMERx(x=0..6)
 * 		TIMER_INT_FLAG_CH0:
 * 			channel 0 interrupt flag, TIMERx(x=0..4)
 * 		TIMER_INT_FLAG_CH1:
 * 			channel 1 interrupt flag, TIMERx(x=0..4)
 * 		TIMER_INT_FLAG_CH2:
 * 			channel 2 interrupt flag, TIMERx(x=0..4)
 * 		TIMER_INT_FLAG_CH3:
 * 			channel 3 interrupt flag, TIMERx(x=0..4)
 * 		TIMER_INT_FLAG_CMT:
 * 			channel commutation interrupt flag, TIMERx(x=0)
 * 		TIMER_INT_FLAG_TRG:
 * 			trigger interrupt flag, TIMERx(x=0)
 * 		TIMER_INT_FLAG_BRK:
 * 			break interrupt flag, TIMERx(x=0)
 */
enum flag_status timer_interrupt_flag_get(uint32_t timer_periph,
		uint32_t interrupt) {

	uint32_t val = TIMER_DMAINTEN(timer_periph) & interrupt;

	if ((TIMER_INTF(timer_periph) & interrupt) != RESET && val != RESET)
		return SET;
	else
		return RESET;
}
