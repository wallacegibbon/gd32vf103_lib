	...
	if (CAN_STAT_IWS != (CAN_STAT(can_periph) & CAN_STAT_IWS)) {
		...
		CAN_BT(can_periph) =
		    (BT_MODE((uint32_t) can_parameter_init->working_mode) |
		     BT_SJW((uint32_t) can_parameter_init->
			    resync_jump_width) | BT_BS1((uint32_t)
							can_parameter_init->
							time_segment_1) |
		     BT_BS2((uint32_t) can_parameter_init->
			    time_segment_2) |
		     BT_BAUDPSC(((uint32_t) (can_parameter_init->prescaler) -
				 1U)));
...
void can_filter_init(struct can_filter_param * can_filter_parameter_init) {
	...
	if (CAN_FILTERBITS_16BIT == can_filter_parameter_init->filter_bits) {
		...
		CAN_FDATA0(CAN0, can_filter_parameter_init->filter_number) =
		    FDATA_MASK_HIGH((can_filter_parameter_init->
				     filter_mask_low) & CAN_FILTER_MASK_16BITS)
		    |
		    FDATA_MASK_LOW((can_filter_parameter_init->
				    filter_list_low) & CAN_FILTER_MASK_16BITS);
		...
		CAN_FDATA1(CAN0, can_filter_parameter_init->filter_number) =
		    FDATA_MASK_HIGH((can_filter_parameter_init->
				     filter_mask_high) & CAN_FILTER_MASK_16BITS)
		    |
		    FDATA_MASK_LOW((can_filter_parameter_init->
				    filter_list_high) & CAN_FILTER_MASK_16BITS);
	}
	/* filter 32 bits */
...
uint8_t can_message_transmit(uint32_t can_periph,
	...
	if (CAN_FF_STANDARD == transmit_message->tx_ff) {
		...
		CAN_TMI(can_periph, mailbox_number) |=
		    (uint32_t) (TMI_EFID(transmit_message->tx_efid) |
				transmit_message->tx_ff | transmit_message->
				tx_ft);
	}
	/* set the data length */
	...
	CAN_TMDATA0(can_periph, mailbox_number) =
	    TMDATA0_DB3(transmit_message->
			tx_data[3]) | TMDATA0_DB2(transmit_message->
						  tx_data[2]) |
	    TMDATA0_DB1(transmit_message->
			tx_data[1]) | TMDATA0_DB0(transmit_message->tx_data[0]);
	CAN_TMDATA1(can_periph, mailbox_number) =
	    TMDATA1_DB7(transmit_message->
			tx_data[7]) | TMDATA1_DB6(transmit_message->
						  tx_data[6]) |
	    TMDATA1_DB5(transmit_message->
			tx_data[5]) | TMDATA1_DB4(transmit_message->tx_data[4]);
	/* enable transmission */
	CAN_TMI(can_periph, mailbox_number) |= CAN_TMI_TEN;
...
/*!
    ...
    \param[out] none
    \retval     enum can_transmit_state
*/
enum can_transmit_state can_transmit_states(uint32_t can_periph,
	...
	case CAN_MAILBOX0:
		val =
		    CAN_TSTAT(can_periph) & (CAN_TSTAT_MTF0 | CAN_TSTAT_MTFNERR0
					     | CAN_TSTAT_TME0);
		break;
		/* mailbox1 */
	case CAN_MAILBOX1:
		val =
		    CAN_TSTAT(can_periph) & (CAN_TSTAT_MTF1 | CAN_TSTAT_MTFNERR1
					     | CAN_TSTAT_TME1);
		break;
		/* mailbox2 */
	case CAN_MAILBOX2:
		val =
		    CAN_TSTAT(can_periph) & (CAN_TSTAT_MTF2 | CAN_TSTAT_MTFNERR2
					     | CAN_TSTAT_TME2);
		break;
	default:
...
/*!
    ...
    \retval     enum err_status: SUCCESS or ERROR
*/
enum err_status can_working_mode_set(uint32_t can_periph, uint8_t working_mode) {
	enum err_status flag = ERROR;
	...
	if (CAN_MODE_INITIALIZE == working_mode) {
		...
		/* enter normal mode */
		CAN_CTL(can_periph) &=
		    ~(uint32_t) (CAN_CTL_SLPWMOD | CAN_CTL_IWMOD);
		/* wait the acknowledge */
		while ((0U !=
			(CAN_STAT(can_periph) &
			 (CAN_STAT_IWS | CAN_STAT_SLPWS))) && (0U != timeout)) {
			timeout--;
		}
		if (0U !=
		    (CAN_STAT(can_periph) & (CAN_STAT_IWS | CAN_STAT_SLPWS))) {
			flag = ERROR;
		} else {
			flag = SUCCESS;
		}
		...
		/* wait the acknowledge */
		while ((CAN_STAT_SLPWS !=
			(CAN_STAT(can_periph) & CAN_STAT_SLPWS))
		       && (0U != timeout)) {
			timeout--;
		}
...
enum can_error can_error_get(uint32_t can_periph) {
	enum can_error error;
	error = CAN_ERROR_NONE;

	/* get error type */
	error = (enum can_error) (GET_ERR_ERRN(CAN_ERR(can_periph)));
	return error;
}
...
/*!
    ...
    \param[in]  can_periph
      \arg        CANx(x=0,1)
    \param[in]  flag: CAN flags, refer to enum can_flag
                only one parameter can be selected which is shown as below:
...
/*!
    ...
    \param[in]  can_periph
      \arg        CANx(x=0,1)
    \param[in]  flag: CAN interrupt flags, refer to enum can_interrupt_flag
                only one parameter can be selected which is shown as below:
...
/*!
    ...
    \retval     none
*/
void can_interrupt_flag_clear(uint32_t can_periph, enum can_interrupt_flag flag) {
